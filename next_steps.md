...
### 1. `CallManager` is doing too much (call_manager.py, 665 lines)
...
### 2. Pervasive `except Exception` swallow-and-log antipattern
...
- `call_manager.py:139, 169, 219, 282, 327, 335, 456, 539, 557, 645` — most of these catch *any* exception from a SIP, audio, or logger operation and just log it.
- `pyvoip_client.py:171, 178, 254, 275, 290, 311, 374, 475` — same pattern.
...
### 3. Race in `PyVoIPClient._call_state_monitor` (pyvoip_client.py:227)
...
### 4. Dead/lossy code in `_on_call_ended` (call_manager.py:577-580)
...
### 5. `send_audio_file` does too much and has a sleep-loop bug (pyvoip_client.py:379-477)
...
### 6. Indentation bug in `MockGPIO.set_input` (gpio_abstraction.py:255-261)
...
### 7. FastAPI deprecation: `@app.on_event` (app.py:159, 175)
...
### 8. Config validation by writing-and-reloading (app.py:273-291)
...
- `audio_handler.py:32-34` — `SAMPLE_RATE`/`FRAME_SIZE` "backward compatibility aliases" for symbols that have never been public. CLAUDE.md says don't add backcompat shims; delete.
- `config_manager.py:18` — `from rotary_phone.exceptions import ConfigError` is mid-module, post-`logger`. Move to the top import block.
- `ConfigManager._normalize_phone_number` (config_manager.py:193) — handles only NANP. The allowlist will silently mis-match international formats. Document or generalize.
- `pyvoip_client._extract_caller_id` (line 355) — `hasattr` duck-typing against a third-party SIP library. If pyVoIP renames `.request.headers`, you silently return "Unknown" forever. Catch a specific `AttributeError` and warn loudly.
- `network_monitor.check_connectivity` opens a TCP socket to `8.8.8.8:53` — DNS doesn't normally accept TCP. Works (Google DNS does answer TCP) but the semantic is "can I reach a thing on the internet," not "is DNS working." Comment or pick a more honest probe.
- Tests use `time.sleep(0.15)` to wait for registration in the `phone_system` fixture (test_integration_e2e.py:86) — flaky on slow CI. Wait on a condition.
...
1. Refactor `CallLogger` to subscribe to events via `_emit_event` — strip ~50 lines of `if self._call_logger:` from `CallManager`.
2. Audit every `except Exception` and either narrow it (use the existing exception hierarchy) or delete it.
3. Fix the `_call_state_monitor` lock race in `PyVoIPClient`.
4. Fix the 0-duration `call_ended` event — read duration before clearing.
5. Switch FastAPI to lifespan; drop `on_event`.
6. Split `_on_off_hook` and `send_audio_file` into single-responsibility helpers.
